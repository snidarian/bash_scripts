...
while getopts :q:x:n:t:h OPT; do
    case $OPT in
	t|t+)
	    target_dir="$OPTARG"
	...
	h|+h)
	    ...
	    ;;
	*)
	    echo "usage: `basename $0` [+-q ARG] [+-n ARG] [+-x ARG] [+-t ARG] [+-h} [--] ARGS..."
	    exit 2
    esac
done
shift `expr $OPTIND - 1`
...
if [[ $multi_round == 1 ]] && [[ $single_round == 0 ]]; then
    ...
    for crf_level in {$min..$max}; do
	for file in *; do
	    echo -e "${green}###########################################################${reset}"
	    echo -e "Now encoding ${red}$file${reset} at CRF $quality"
	    echo -e "${green}###########################################################${reset}"
	    ffmpeg -i "$file" -vcodec libx265 -crf $crf_level ~/Videos/staging/"$file"$quality.mkv;
	done
