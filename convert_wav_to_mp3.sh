...
# The first argument to the script should be the parent filepath that houses the .wav files you
...
# issue find command and store the file paths of all "offending" files (.wav files)

offenders="$(find $BASEFILEPATH -type f -name "*.wav")"
...
do
    ...
    new_filename=$(sed 's/.wav/.mp3/' <<< $file)
    # Save the previous .wav file to temporary variable so it can be deleted after the conversion
