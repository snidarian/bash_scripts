...
# The first argument to the script should be the parent filepath that houses the .flac files you
...
# issue find command and store the file paths of all "offending" files (.flac files)

offenders="$(find $BASEFILEPATH -type f -name "*.flac")"
...
do
    ...
    new_filename=$(sed 's/.flac/.mp3/' <<< $file)
